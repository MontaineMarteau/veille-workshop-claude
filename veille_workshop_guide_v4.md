# Veille hebdomadaire — tous domaines (v4)
...
- **Pour des options finies et stables** (langue/zone, cadence, priorité « ne rien rater » vs « ne lire que l'essentiel », fréquence) : utiliser l'**interface de choix cliquables** (AskUserQuestion). 1 clic, zéro friction, le participant ne tape rien.
- **Pour des options ouvertes où le participant doit pouvoir amender ou ajouter** (sous-axes du domaine, ruptures, allergies, éclaireurs, améliorations à apporter) : rester en **conversation textuelle** avec propositions dans le chat. Le participant peut biffer, ajouter, reformuler.
...
Avant de partir chercher les sources, comprenez en deux questions courtes le contexte du participant, dans une formulation fluide (pas un formulaire RH) :
...
- **Le rôle** calibre le « so what » (un PM, un CEO, un dev, une RH ne tirent pas la même implication d'un même fait).
- **Le niveau** calibre le contenu et le glossaire. Débutant·e → on garde les bases utiles, on enrichit le glossaire. Senior → on bannit les bases vues 1000 fois et on va plus pointu, on allège le glossaire.

Si la personne hésite à se classer en niveau, proposer les 3 options en cliquable (débutant·e / intermédiaire / avancé·e).

#### 1.5 Langue/zone et cadence

Demander la **langue/zone géographique prioritaire** (par défaut : la langue de la conversation + international), et la **cadence** (défaut : vendredi 17h). Ces deux questions sont à choix finis et stables → format cliquable.
...
- un **titre court**,
- 2-3 phrases d'**idée clé** (calibrées au niveau d'expertise du participant, cf. 1.4),
- un **« so what »** (implication concrète, **calibrée au rôle** du participant, 1 phrase),
...
4. **Compléter** l'objet `GLOSSARY` avec les termes spécifiques du domaine. Calibrer la densité du glossaire au niveau du participant (débutant·e → plus d'entrées avec définitions de base ; senior → moins d'entrées, uniquement les termes vraiment spécialisés).
...
Le « so what » est calibré au rôle du participant : [RÔLE]. Le niveau d'expertise est
[NIVEAU], en tenir compte pour la profondeur des items et la densité du glossaire.
...
- **Calibré au rôle et au niveau du participant** — c'est ce qui rend la veille personnelle, pas générique.
